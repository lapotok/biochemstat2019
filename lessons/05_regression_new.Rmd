...
dd %>% 
  ...
  geom_segment(aes(x=2.75, xend=2.75, y=group_means[[2,2]], yend=group_means[[3,2]]), col = "green", size=.15, arrow = arrow(length=unit(0.2,"cm"), ends="last")) +
  ...
  annotate("text", x=1.5, y=(group_means[[1,2]]+group_means[[2,2]])/2, label=latex2exp::TeX("$\\beta_{low\\rightarrow average}$"), col="blue") +
...
# Нелинейная регрессия (НЕ ЗАКОНЧЕНО)
...
```{r, eval=F}
load("data/dr_data.RData")

nt_igfbp4_1 %>% 
  ggplot()+
  geom_point(aes(x=dilution, y=signal, col=sample_id)) +
  scale_y_log10() + scale_x_log10()

calibr = nt_igfbp4_1 %>% filter(sample_id == "NT-IGFBP4 (HEK)" & dilution != Inf)
...
# c = low, d = up, b = slope, e = offset
m3.0 = nls(signal ~ c+(d-c)/(1+exp(b*(log(dilution)-e))), 
           data=calibr, 
           start=list(d=1e6, c=1e2, e=1, b=4), trace=TRUE)
summary(m3.0)
# inverse prediction: exp(log(((d - c)/(signal - c)) - 1)/b + log(e))

library(drc)
m3.1 = drm(signal ~ dilution, data = calibr, fct = LL.4())
m3.1
ED(m3.1, c(1e4, 2e5), type = "absolute", interval = "delta")
```

# Корреляция (НЕ ЗАКОНЧЕНО)
...
# var types
BrendonSmall_nutrition %>% head()
BrendonSmall_nutrition_num = BrendonSmall_nutrition[,c("Grade", "Weight", "Calories", "Sodium", "Score")]
...
# r and p-values
cor.test(BrendonSmall_nutrition_num[,1], BrendonSmall_nutrition_num[,2])

library(PerformanceAnalytics)
chart.Correlation(BrendonSmall_nutrition_num, method = "pearson")

# plotting them all
library(corrplot)
iris_cor_p = cor.mtest(iris[, 1:4], conf.level = .95)$p
corrplot::corrplot.mixed(cor(iris[, 1:4]), upper="ellipse", p.mat=iris_cor_p)
chart.Correlation(iris[, 1:4], method = "pearson")
```